...
redDiagLog = format["MONEY: Игрок: %1 (%2) снял из банка %3 %4. Было %5, стало %6 %4 log", player, (getPlayerUID player), _amount, CurrencyName, _wealth, (player getVariable['cashMoney',0])];
...
BankDialogDepositAmount = {
	...
	redDiagLog = format["MONEY: Игрок: %1 (%2) положил в банк %3 %4. Было %5, стало %6 %4 log", player, (getPlayerUID player), _amount, CurrencyName,  _wealth, (player getVariable['cashMoney',0])];
	publicVariable "redDiagLog";		
	};
	PVDZE_plr_Save = [player,(magazines player),true,true] ;
	publicVariableServer "PVDZE_plr_Save";
...
GivePlayerAmount = {
	...
	redDiagLog = format["MONEY: Игрок: %1 (%2) дал %3 %4 игроку %5 (%6) log", player, (getPlayerUID player), _amount, CurrencyName, _target, (getPlayerUID _target)];
	publicVariable "redDiagLog";
	uiSleep 0.5;
	redDiagLog = format["MONEY: У игрока %1 (%2) осталось %3 %4. У игрока %5 (%6) стало %7 %4 log", player, (getPlayerUID player), (player getVariable['cashMoney',0]), CurrencyName, _target, (getPlayerUID _target), (_target getVariable['cashMoney',0])];
	publicVariable "redDiagLog";	
};