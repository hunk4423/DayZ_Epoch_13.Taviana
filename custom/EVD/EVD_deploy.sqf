...
if (dayz_combat == 1) exitwith { cutText ["\n\nVocê está em combat, não pode construir agora!", "PLAIN DOWN"] };
	...
	if (_proceed) then {
		...
		if (_finished) then {
			...
			if(_removed_total == _tobe_removed_total) then {
				...
				cutText ["\n\nCUIDADO: Se você desmontar a caixa, todos os objetos dentro dela irão sumir!", "PLAIN DOWN",10];
			} else {
		...
		cutText [format["Está faltando %1 %2",_missingQty, _textMissing], "PLAIN DOWN"];
	};
} else {
	cutText ["\n\nCanceled deploying vehicle", "PLAIN DOWN"];
};